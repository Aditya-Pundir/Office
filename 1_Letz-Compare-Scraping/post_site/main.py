...
csv_headers = ("Device ID", "Memory", "Subscription", "Available For Online Sale", "Price Without Subscription", "Final Price",
               "Color ID", "Color Code", "Article ZID", "Article ID", "Color Available For Online Sale", "Root Code", "Brand", "Model", "Package ID", "Commitment ID", "Commitment Duration", "Phone Option Price", "Monthly Price")
...
def post_scraper():
    ...
    monthly_prices = driver.find_elements(
        By.CLASS_NAME, "card-mobile-phone-plan-price")
    mobile_phones = []
    ...
    #     f"""document.getElementsByClassName("custom-control-label")[0].scrollIntoView();""")
    # buttons[0].click()
    ...
    for plan in range(len(plans)):
        ...
        monthly_price = monthly_prices[plan].get_attribute(
            "innerText").replace("\n", ".", 1).split("\n")[0]
