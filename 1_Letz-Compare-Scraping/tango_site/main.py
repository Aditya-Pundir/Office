        ...
        for phone in phones:
            ...
            for parent_key in list(phone[2]["colors"].keys()):
                for key in list(phone[2]["colors"][parent_key].keys()):
                    if phone[2]["colors"][parent_key][key]["title"] not in color_titles:
                        colors.append(phone[2]["colors"][parent_key][key])
                        color_titles.append(
                            phone[2]["colors"][parent_key][key]["title"])
            for i, color in enumerate(colors):
                to_append[11]["Color Title"].append(color_titles[i])
